"""
Post-process ACTINN predictions: compute accuracy, macro-F1, weighted-F1,
per-class F1, and confusion matrices for 8K and 3K query donors.

ACTINN is trained on the 5-class coarse taxonomy directly (same as Seurat),
so only the ground-truth labels need mapping — predictions are native.
"""
import os
...
# Ground-truth external → coarse (identical mapping to SingleR/Seurat postprocess)
GT_LABEL_MAP = {
    "B cells":  "B cells",
    "Mono":     "Mono",
    "CD8 T":    "T cells",
...
def evaluate(ds_name, processed_path, pred_path):
    ...
    # ACTINN predictions are native 5-class; "Other" should not occur,
    # but we keep the column for symmetry with the other comparators.
    ...
    cm_df.to_csv(OUTDIR / f"actinn_{ds_name.lower()}_confusion.csv")
...
def main():
    metrics = {"tool": "ACTINN_pytorch_reimpl"}
    metrics["8K"] = evaluate(
        "8K",
        "data/processed/pbmc8k_labeled.h5ad",
        OUTDIR / "actinn_8k_predictions.csv",
    )
    metrics["3K"] = evaluate(
        "3K",
        "data/processed/pbmc3k_labeled.h5ad",
        OUTDIR / "actinn_3k_predictions.csv",
    )
    run_info_path = OUTDIR / "actinn_run_info.csv"
    if run_info_path.exists():
        metrics["run_info"] = pd.read_csv(run_info_path).to_dict(orient="records")

    with open(OUTDIR / "actinn_metrics.json", "w") as f:
        json.dump(metrics, f, indent=2)
    print(f"\n✅ Metrics saved to {OUTDIR / 'actinn_metrics.json'}")
