"""
Post-process scANVI predictions returned from Google Colab.
...
Produces:
  scanvi_{8k,3k}_confusion.csv
  scanvi_metrics.json

scANVI is trained on 5-class coarse labels (68K), so predictions are native
5-class.  Ground-truth labels in 8K/3K are mapped identically to the other
comparators before evaluation.
"""
import os
...
OUTDIR = Path("results/comparators/scanvi")
...
def evaluate(ds_name, processed_path, pred_path):
    ...
    cm_df.to_csv(OUTDIR / f"scanvi_{ds_name.lower()}_confusion.csv")
