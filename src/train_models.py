...
# subsampling / feature selection settings (speed baselines)
MAX_CELLS = 20000
N_TOP_GENES = 2000
...
def load_data(
    ...
    # Optionally subsample cells for faster baselines
    if max_cells is not None and adata.n_obs > max_cells:
        rng = np.random.RandomState(RANDOM_STATE)
        idx = rng.choice(adata.n_obs, size=max_cells, replace=False)
        adata = adata[idx].copy()

    # Highly variable genes (no scikit-misc dependency)
    ...
    y = cell_type_cat.cat.codes.to_numpy()
...
def split_and_scale(X, y, test_size: float = TEST_SIZE, random_state: int = RANDOM_STATE):
    ...
    scaler = StandardScaler(with_mean=False)  # required for sparse
...
def train_xgboost(X_train, y_train, X_test, y_test, num_classes: int, output_dir: str):
    """
    Train XGBoost baseline and save model safely for macOS.

    IMPORTANT:
    - Avoid joblib/pickle for XGBoost on some macOS setups (can cause segfaults later).
    - Save with xgb_model.save_model(...) (JSON) instead.
    """
    ...
    # Save safely (avoid joblib for xgboost)
...
def save_metrics_and_reports(
    ...
    report = classification_report(y_test, y_pred_xgb, target_names=label_names)
    ...
    print(f"Saved XGBoost model to {os.path.join(output_dir, 'xgb_model.json')}")
...
def main():
    ...
    _, acc_lr, f1_lr, _ = train_logistic_regression(X_train, y_train, X_test, y_test, OUTPUT_DIR)
    pbar.set_postfix_str(f"LR acc {acc_lr:.3f}, F1 {f1_lr:.3f}")
    pbar.update(1)

    # 4) Train XGBoost
    pbar.set_description(f"4/5 {steps[3]}")
    _, acc_xgb, f1_xgb, y_pred_xgb = train_xgboost(
